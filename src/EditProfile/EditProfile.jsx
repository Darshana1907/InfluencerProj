...
const PlatformComp = () => {
  ...
  const handleRedirect = (url, platform) => {
   ...
   const handleNext = () => {
    ...
    navigate('/dashboard'); // You can replace '/dashboard' with your target URL
  };

  return (
    <div className="fixed inset-0 h-[1600px] top-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-[710px] h-[790px] z-10  top-[24%] rounded-[10px] absolute left-1/2 -translate-x-[50%] -translate-y-[50%] bg-white shadow-[2px_4px_14px_2px_rgba(0,0,0,0.25)]">
        ...
        <div className='w-[584px] h-[391px] ml-16 mt-10'>
          {data.map((item, index) => (
            ...
            >
              <img src={item.img} className='w-[22px] h-[22px] rounded-[5px]' alt={item.title} />
              <p className='w-[212px] h-[23px] text-[18px] font-semibold font-body text-[#363939]'>{item.title}</p>
            </div>
          ))}
        </div>
        
        <div className='mt-10 text-end mr-16'>
          <button onClick={handleNext} className='w-[170px] h-[45px] px-[16px] bg-[#0066ff] rounded-[8px] text-[16px] font-normal font-body text-white'>Next</button>
        </div>
      </div>
    </div>
  );
